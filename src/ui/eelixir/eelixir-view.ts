...
export class EelixirView {
  ...
  private renderData: EelixirRenderData | undefined;
   ...
   */
  init(uiCtx: EelixirUIContext): void {
    this.uiCtx = uiCtx;
   ...
   */
  async onSwitchIntent(agentId: string, targetAgentId: string): Promise<boolean> {
    // Validate switch
   ...
   */
  private async startNextAgent(): Promise<void> {
    const nextAgent = this.manager.getNextAgent();
    if (nextAgent) {
   ...
   * Start agent
   */
  private async startAgent(agent: EelixirAgentState, reason: string): Promise<void> {
   ...
   */
  private async onAgentComplete(agentId: string, result: string): Promise<void> {
    if (!this.uiCtx) return;
   ...
   * Get agents list
   */
  public getAgents(): EelixirAgentState[] {
