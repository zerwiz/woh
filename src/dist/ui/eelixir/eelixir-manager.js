 ...
 * - Session persistence
 */
import { formatTokens, } from "./eelixir-agent-types";
...
export class EelixirAgentManager {
     ...
     * Start an agent
     */
    async startAgent(agentId, maxTurns) {
    ...
    getNextAgent() {
        ...
        const activeOrStopped = this.getAgents().filter((a) => (a.status === "active" || a.status === "stopped") && !a.session?.disabled);
     ...
     */
    async executeTool(agentId, toolName, toolResult) {
        const agent = this.agentList.get(agentId);
        if (!agent) {
            return false;
        }
...
//# sourceMappingURL=eelixir-manager.js.map