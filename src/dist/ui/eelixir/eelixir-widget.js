...
export class EelixirWidget {
    ...
    categorizeAgents() {
        ...
        return {
            ...
            finished: agents.filter((a) => a.status !== "running" &&
                a.status !== "queued" &&
                a.status !== "completed" &&
    ...
    renderWidget(tui, theme) {
        ...
        const truncateLine = (line) => truncate(line, cols);
        ...
        const parts = [
            truncateLine(theme.fg(headingColor, headingIcon) +
        ...
        if (queued.length > 0) {
            const queuedLine = truncateLine(theme.fg("dim", "◦") +
        ...
        if (parts.length > MAX_WIDGET_LINES) {
            ...
            parts.push(truncateLine(theme.fg("dim", "─") +
    ...
    renderFinishedAgentLine(agent, theme, cols) {
        const icon = theme.fg(STATUS_COLORS[agent.status], ICONS[agent.status]);
        ...
        const parts = [];
        if (agent.metadata?.turnCount)
            parts.push(formatTurns(agent.metadata.turnCount, agent.metadata.maxTurns));
        if (toolUses > 0)
            parts.push(formatToolUsage(toolUses));
        if (tokens)
            parts.push(tokens);
        parts.push(duration);
        return truncateLine(theme.fg("dim", "─") +
    ...
    renderRunningAgentLines(agent, theme, cols) {
        const headerParts = [];
        if (agent.metadata?.turnCount)
            headerParts.push(formatTurns(agent.metadata.turnCount, agent.metadata.maxTurns));
        const toolUses = agent.activeTools?.size || 0;
        if (toolUses > 0)
            headerParts.push(formatToolUsage(toolUses));
        const tokens = agent.session?.tokens ? formatTokens(agent.session.tokens) : "";
        headerParts.push(tokens);
        headerParts.push(formatDuration(agent.session?.startedAt));
        const header = truncateLine(theme.fg("dim", "─") +
        ...
        return [header, truncateLine(theme.fg("dim", "│  ") + theme.dim(activityParts.join(" ")))];
    }
    ...
    render(tui) {
        return {
            render: () => this.renderWidget(tui, tui.theme),
            invalidate: () => {
...
//# sourceMappingURL=eelixir-widget.js.map