 ...
 */
async function onAgentError(agent, error, ctx) {
    // MIGRATION: Use persistent counter with 10s timeout
 ...
 */
function handleStopKey(data, matchesKey, ctx) {
 ...
 */
function incrementRunningAgentCount() {
    _runningAgentsCount++;
}
/**
 ...
 */
function decrementRunningAgentCount() {
    _runningAgentsCount--;
}
/**
 ...
 */
function get_runningAgentCount() {
 ...
 */
function updatePersistentCounter(status) {
...
export class AgentManager {
     ...
     * Create AgentManager
     */
    constructor(config, agents, ctx) {
        this.config = config;
        this._agents = agents;
        this.ctx = ctx;
        this.stopKeyConfig = {
            key: config.stopKey,
            matchesFunction: (key, data) => /* Implementation specific to key matching */ key === config.stopKey,
        };
    }
    /**
    ...
    registerStopKey() {
        if (!this.ctx)
            return;
        // MIGRATION: Debouncing for stop keys
        handleStopKey({ key: this.stopKeyConfig?.key || "" }, this.stopKeyConfig?.matchesFunction || (() => false), this.ctx);
    }
    /**
     * Stop all running agents
     ...
     * Agent error handled
     */
    async onAgentErrorComplete(agent, error) {
 ...
 */
export function createMigratedManager() {
    const ctx = {
        setStatus: (key, status) => {
            console.log(`Status ${key} set to ${status}`);
        },
        notify: (key, message, duration) => {
